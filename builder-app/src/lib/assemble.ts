...
export async function assembleSite(
  ...
  // Copy uploaded logos/images so they are bundled with export and deployment
  const uploadsCandidates = [
    join(process.cwd(), 'public/uploads'),
    join(process.cwd(), 'builder-app/public/uploads'),
    resolve(__dirname, '../../public/uploads'),
    resolve(__dirname, '../../../public/uploads'),
  ];
  for (const upDir of uploadsCandidates) {
    if (existsSync(upDir)) {
      copyDir(upDir, join(destDir, 'public/uploads'));
      break;
    }
  }
...
${content.airwallexCheckoutUrl ? '' : `## Checkout

The checkout page is currently in **contact-only mode** — it does not collect card
details or process payments. To enable live payments, add your Airwallex Hosted
Payment Page URL to \`content.airwallexCheckoutUrl\` in \`src/content/site.ts\`
(or connect it from the studio's Publish panel) and redeploy.

---

`}## Airwallex Payment Onboarding Checklist

This site is generated to satisfy Airwallex's payment-method onboarding
requirements (https://www.airwallex.com/docs/payments/payment-methods/payment-method-onboarding-requirement).
Before applying for payment-method activation, verify:

1. **Business identification details** — company name, registration number,
   registered address, email, and phone are in the footer and policy pages.
2. **Website policies** — Terms & Conditions, Privacy Policy, Shipping and
   Refund policies are live under \`/policies/*\`.
3. **Governing law** — the Terms of Service are governed by the law of the
   business's registered country (${content.business?.jurisdiction ?? 'the registration jurisdiction'}). If you change
   registration, update \`business.governingLaw\` and \`business.jurisdiction\`.
4. **Checkout experience** — clear pricing on product pages; checkout must
   complete without errors before you apply.
5. **No external redirection** — checkout only redirects to the Airwallex
   hosted payment page, never to any other site.
6. **Cloudflare Email Address Obfuscation** — if the domain is proxied through
   Cloudflare, **disable Email Address Obfuscation** (Cloudflare Dashboard →
   Scrape Shield → Email Address Obfuscation → Off). Otherwise the support
   email is encoded in the HTML and static reviewers may not see it.
7. **Industry alignment** — the site's visible products/services must match
   the merchant category declared on the Airwallex application.
